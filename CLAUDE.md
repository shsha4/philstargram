...
- **phase 2 까지 완료** (v1 MVP + Spring Modulith 이벤트 기반 fan-out/notification).
- 다음은 **phase 3** (Outbox + 전술 DDD 개선). 로드맵은 `README.md` 의 "로드맵" 절이
  기준이며, **인프라 단계마다 도메인 개선을 인터리빙**하는 개정판을 따른다.
...
## 알려진 개선 과제 (phase 3 대상)

아키텍처 리뷰 결과: 헥사고날/경계는 강하나 **전술 DDD 가 약함**(빈혈 도메인). phase 3 에서
함께 해소할 것:
- 도메인 이벤트를 **애그리거트가 raise** (현재는 UseCase 가 발행)
- 값 객체 `Email`/`Nickname` (현재 형식 검증을 web DTO `@Email` 에 위임)
- 타입드 ID `MemberId`/`PostId`/`FollowId` (현재 raw `Long`)
- 이벤트 리스너를 `adapter.in.event` + `FanOutFeedUseCase` 로 분리 (web 인입과 일관성)
- 중복 규칙 제거 (게시글 길이 제한이 `Post.write` 와 request DTO 양쪽에 하드코딩)
