...
- **phase 5 진행 중.** 5a·5b 완료. 남은: 5c(하이브리드 팬아웃 + Kafka Streams follower-count).
  - ✅ **5a Redis 피드 캐시**: `FeedCache` 아웃바운드 포트 + `RedisFeedCache` 어댑터(ZSET).
    ...
    (bounded staleness). 설계 근거는 `LEARNINGS.md` 2026-07-04 절.
  - ✅ **5b 컨슈머 idempotency**: at-least-once 재전달 중복을 자연 멱등성(A)으로 흡수. feed 는
    ...
    트랜잭션 abort 라 미사용). `NotifyNewPostCommand` 에 postId 추가. 근거는 `LEARNINGS.md`.
