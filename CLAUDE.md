...
- **phase 3 진행 중** (v1 MVP + Spring Modulith 이벤트까지 완료된 위에서).
  ...
  - ⬜ 값 객체 `Email`/`Nickname`
  - ⬜ 타입드 ID `MemberId`/`PostId`/`FollowId` (모듈 간 참조 타입 결정 포함)
  - ⬜ 도메인 이벤트를 애그리거트가 raise
...
docker compose up -d     # 로컬 실행용 PostgreSQL (localhost:5432)
./gradlew.bat bootRun    # 앱 기동 (schema.sql 자동 실행)
```
...
## 남은 phase 3 개선 과제 (전술 DDD)

아키텍처 리뷰 결과: 헥사고날/경계는 강하나 **전술 DDD 가 약함**(빈혈 도메인). phase 3 에서
함께 해소 중이며, **남은 것**은 도메인 파급이 큰 아래 셋:
- 값 객체 `Email`/`Nickname` (현재 형식 검증을 web DTO `@Email` 에 위임). 도메인이 스스로
  형식/불변식을 보장하도록. Result/Summary/영속성 매핑은 String 유지.
- 타입드 ID `MemberId`/`PostId`/`FollowId` (현재 raw `Long`). **설계 결정 필요**: 모듈 간
  참조(post 의 authorId 등)를 타입드로 하면 모듈 간 타입 결합이 생김 → 모듈 내부 식별자만
  타입드로 하고 모듈 경계는 raw `Long` 유지하는 쪽을 우선 검토. JSON 직렬화/JPA 매핑 영향 확인.
- 도메인 이벤트를 **애그리거트가 raise** (현재는 UseCase 가 발행). 도메인을 framework-free 로
  유지해야 하므로 Spring Data `AbstractAggregateRoot`(JPA 결합) 금지 — 애그리거트가 이벤트를
  내부 리스트에 누적하고 UseCase/adapter 가 드레인 후 발행하는 자체 패턴 사용.

완료분: Outbox, 게시글 길이 중복 제거, 이벤트 리스너 `adapter.in.event` 분리 (위 "현재 상태" 참고).
