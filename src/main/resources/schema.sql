...
CREATE TABLE IF NOT EXISTS posts (
    ...
    -- 작성 시점 작성자 닉네임 스냅샷(비정규화). 읽기 시점 pull(하이브리드 팬아웃, phase 5c)에서
    -- feed 가 셀럽 글을 당겨올 때 member 를 재조회하지 않도록 post 가 함께 들고 있는다.
    author_nickname VARCHAR(50)  NOT NULL,
    content         VARCHAR(2000) NOT NULL,
    created_at      TIMESTAMP    NOT NULL
);
-- 읽기 시점 pull: 셀럽(작성자)들의 최근 글을 작성자 id 로 모아 최신순 조회.
